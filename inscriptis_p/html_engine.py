...
from inscriptis_p.css import CSS, CssParse, HtmlElement
from inscriptis_p.html_properties import Display, WhiteSpace, Line
from inscriptis_p.table_engine import Table
...
class Inscriptis(object):
    ...
    # UL_COUNTER = ('* ', '+ ', 'o ', '- ')
    UL_COUNTER = ('[UnorderedList=1] ', '[UnorderedList=2] ', '[UnorderedList=3] ', '[UnorderedList=4] ')
    ...
    def __init__(self, html_tree, display_images=False, deduplicate_captions=False, display_links=False):
        ...
        self.start_tag_handler_dict = {
            ...
            'aside': self.start_aside,
            'nav': self.start_nav,
            'footer': self.start_footer,
            'iframe': self.start_iframe,
        ...
        self.end_tag_handler_dict = {
            ...
            'code': self.end_code,
            'p': self.end_p,
            'video': self.end_video,
            'aside': self.end_aside,
            'nav': self.end_nav,
            'footer': self.end_footer,
            'iframe': self.end_iframe,
    ...
    def get_text(self):
        ...
        '''
        unescaped = unescape(' [Newline] '.join(chain(*self.clean_text_lines))).rstrip()
        unescaped = "".join([' [Newline] ', unescaped])
        return unescaped
    ...
    def start_img(self, attrs):
        ...
        if image_text and not (self.cfg_deduplicate_captions and image_text == self.last_caption):
            self.current_line[-1].content += ' [StartImage] {} [EndImage]'.format(image_text)
    ...
    def start_p(self, attrs):
        ...
        self.current_line[-1].content += ' [Paragraph] '


    def end_p(self):
        #self.link_target = attrs.get('href', '')

        if self.clean_text_lines[-1][-1] == "[Paragraph]":
            
            self.clean_text_lines[-1].pop()



    def start_a(self, attrs):
        self.link_target = attrs.get('href', '')
        if self.link_target.startswith("http"):
            self.link_type = " [WebLinkStart] "
        elif self.link_target.startswith("#"):
            self.link_type = " [LocalLinkStart] "
        else:
            self.link_type = " [RelativeLinkStart] "



        self.current_line[-1].content += ' {} '.format(self.link_type)

    def start_video(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [VideoStart] '

    def end_video(self):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [VideoEnd] '

    def start_title(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [TitleStart] '

    def start_nav(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [NavStart] '

    def start_aside(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [AsideStart] '

    def start_footer(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [FooterStart] '

    def start_iframe(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [IframeStart] '

    def end_iframe(self):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [IframeEnd] '

    def end_nav(self):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [NavEnd]'

    def end_aside(self):
        #self.link_target = attrs.get('href', '')
        
        self.current_line[-1].content += ' [AsideEnd] '

    def end_footer(self):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [FooterEnd] '

    def end_a(self):
        self.link_type = self.link_type.replace("Start", "End")
        self.current_line[-1].content += self.link_type

    def start_code(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [CodeStart] '

    def start_h1(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [H1Start] '
    
    def start_h2(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [H2Start] '

    def start_h3(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [H3Start] '

    def start_h4(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [H4Start] '

    def start_h5(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [H5Start] '

    def start_h6(self, attrs):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [H6Start] '    
    
    def end_code(self):
        #self.link_target = attrs.get('href', '')
        self.current_line[-1].content += ' [CodeEnd] '
    ...
    def start_li(self, attrs):
        ...
        if self.li_level > 0:
            ...
            bullet = " [UnorderedList=1] "
        if isinstance(bullet, int):
            self.li_counter[-1] += 1
            self.current_line[-1].list_bullet = " [OrderedList] {}. ".format(bullet)
        else:
    ...
    def end_li(self):
        ...
        if isinstance(bullet, int):
            ...
            endlist = " [OrderedList] {}. ".format(bullet - 1)
        else:
            endlist = bullet
        endlist = endlist.replace("List", "ListEnd")
        
        self.clean_text_lines[-1][-1] +=" {}".format(endlist)


    def start_table(self, attrs):
        self.current_line[-1].content += ' [TableStart] '

    def start_tr(self, attrs):
        self.write_line()
        self.current_line[-1].content += ' [RowStart] '

    def start_td(self, attrs):
        self.current_line[-1].content += ' [CellStart] '

    
    def end_table(self):
        self.current_line[-1].content += ' [TableEnd] '

    def end_tr(self):
        self.current_line[-1].content += ' [RowEnd] '

    def end_td(self):
        self.current_line[-1].content += ' [CellEnd] '
    ...
    def newline(self, attrs):
        self.current_line[-1].content += " [LineBreak] "
        self.write_line(force=True)

...
class BertNQA(BertPreTrainedModel):
    """BERT model for Question Answering (span extraction).
    This module is composed of the BERT model with a linear layer on top of
    the sequence output that computes start_logits and end_logits
    ...
    """
    def __init__(self, config):
        super(BertNQA, self).__init__(config)
        self.bert = BertModel(config)
        ...
        self.answer_type = nn.Linear(config.hidden_size, num_answer_types)


        self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        sequence_output, pooler = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, answer_type=None)
        logits = self.qa_outputs(sequence_output)
        ...
        if start_positions is not None and end_positions is not None and answer_type is not None:
            # If we are on multi-GPU, split add a dimension
            if len(start_positions.size()) > 1:
                start_positions = start_positions.squeeze(-1)
            if len(end_positions.size()) > 1:
            ...
            start_loss = loss_fct(start_logits, start_positions)
            end_loss = loss_fct(end_logits, end_positions)
            answer_type_loss = a_loss_fct(answer_type_logits, answer_type)

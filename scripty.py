...
resp = requests.get('http://index.commoncrawl.org/CC-MAIN-2015-27-index?url=http%3A%2F%2Fhttps://www.kurzweilai.net/essentials-of-general-intelligence-the-direct-path-to-agi%2F&output=json')
pages = [json.loads(x) for x in resp.content.strip().split(b'\n')]
# Multiple pages may have been found - we're only interested in one
page = pages[0]


indextime = time.time() - start_time
print(f"Downloading finished after {indextime} seconds")
